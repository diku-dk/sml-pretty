...
struct
  ...
  in
    ...
    fun around left right doc =
      block1 ((group (left ^^ doc)) ^^ right)

    fun opt ppr NONE = Pretty.empty
      | opt ppr (SOME x) = ppr x

    fun list left right sep elem =
      let
        fun elements [a] = elem a ^^ right
          | elements (a :: s) =
              group (elem a ^^ sep) ^^ elements s
          | elements _ = raise Fail "Impossible"

        fun list nil = left ^^ right
          | list l =
              left ^^ nest 1 (elements l)
      in
        list
      end
