...
struct
    ...
    fun fitting [] left = true
      | fitting ((i, mode, doc) :: rest) left =
          if left >= 0 then
            ...
            | APPEND (x, y) =>
                fitting ((i, mode, x) :: (i, mode, y) :: rest) left
      ...
      let
        fun be s k [] = s
          | be s k ((i, mode, doc) :: rest) =
              ...
              | TEXT str =>
                  let val s = outs s str
                  in be s (k + String.size str) rest
                  end
              | NEWLINE => let val s = nlspace outs s i in be s i rest end
              | BREAK (sp, off) =>
                  (case mode of
                     Flat =>
                       let val s = spaces outs s sp
                       in be s (k + sp) rest
                       end
                   | Break =>
                       let val s = nlspace outs s (i + off)
                       in be s (i + off) rest
                       end)
              | GROUP x =>
                  (case mode of
                   ...
                   | Break =>
                       if fitting ((i, Flat, x) :: rest) (w - k) then
                         be s k ((i, Flat, x) :: rest)
                       else
                         be s k ((i, Break, x) :: rest))
      in
        be s 0 [(0, Break, x)]
      end
  end
  ...
  fun toFile w filename doc =
    ...
    in
      (toOutStream w dev doc)
      handle ? => (TextIO.closeOut dev; raise ?);
      TextIO.closeOut dev
    end
